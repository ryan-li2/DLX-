...
for z in [4,8,12]:#4:Realistic, 8:Random, 12:Clustered
    for INS in [500,1000,2500,5000]:#Number of users
        ...
        for h in range(20): #problems     
            ...
            pairs_file = os.path.join('/Data/Beam Candidate/Pairs/', 'Pairs_'+str(z)+'_'+str(INS)+'_sample'+str(h)+'.json')
...
# Map backtrack → class index (0 to 5)
label_map = {b: i for i, b in enumerate(BACKTRACKS)}

# Inverse map: class index → backtrack
inv_label_map = {i: b for b, i in label_map.items()}
